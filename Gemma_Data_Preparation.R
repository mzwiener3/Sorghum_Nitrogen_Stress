...
write.table(ph[,2:7], row.names = F, col.names = F, quote = F, sep = "\t", file = "gemma/phenotype.txt")

# Phenotype names for naming output files from loop (if you want to is it, it is not necessery.)
write.table(colnames(ph)[2:7], row.names = F, col.names = F, quote = F, sep = "\t", file = "gemma/phenotypeNames.txt")
...
write.table(pc, row.names = F, col.names = F, quote = F, sep = "\t", file = "gemma/covariate.txt")
...
dat[head(order(dat$p_lrt), n=15),] # look at 10 top associated SNPs
  ...
  ##terminal
./gemma -bfile ../data/plink/SAP -p phenotype.txt -gk 1 -o SAPKin
...
a=$(sed -n ${i}p phenotypeNames.txt)

echo ${a}



./gemma -bfile ../data/plink/SAP -p phenotype.txt -k SAPKin.cXX.txt -c covariate.txt  -lmm 2 -n ${i} -o ${a}
